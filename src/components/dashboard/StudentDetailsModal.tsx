    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-lg w-full relative">
        ...
        <div className="space-y-2">
          ...
          {student.notes && <div><strong>Notes:</strong> {student.notes}</div>}
          <div><strong>Preferred Mode:</strong> {student.preferredMode}</div>
          <div><strong>Enrolled Courses:</strong> {Array.isArray(student.enrolledCourses) ? student.enrolledCourses.map((c: any) => c.title || c).join(', ') : ''}</div>
          <div><strong>Registered At:</strong> {new Date(student.createdAt).toLocaleString()}</div>
        </div>
      </div>
    </div>
  );
